/*
  MaUWB_ESP32S3-with-STM32 Tag 指令範例程式碼
  使用的函式庫版本:
  - Wire: 2.0.0
...
#define UWB_INDEX 0        // 定義 UWB 裝置編號，可以調整為不同裝置的 ID，這裡設為 0
#define TAG                // 定義此裝置為 Tag 角色，Tag 是可移動的設備
#define FREQ_850K          // 設置使用 850k 頻率，這裡可以選擇 850k 或 6.8M 頻率
#define UWB_TAG_COUNT 64   // UWB 支持的 Tag 數量，根據實際需求可以調整
...
void logoshow(void) {
    ...
    String temp = "";
    temp = temp + "T" + UWB_INDEX;      // 顯示 UWB ID
    temp = temp + "   850k";            // 顯示頻率
...
String config_cmd() {
    ...
    temp = temp + ",0";          // 設置裝置角色為 Tag (0 = Tag, 1 = Anchor)
    temp = temp + ",0";          // 設置頻率為 850k (0 = 850k, 1 = 6.8M)
